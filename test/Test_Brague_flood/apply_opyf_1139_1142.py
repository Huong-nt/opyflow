...
video = opyf.videoAnalyzer(os.path.join(CURR_PATH, '1139/IMG_1139.MOV'))
...
mask = cv2.imread(os.path.join(CURR_PATH, '1139/mask_1139.png'))
A = mask < 100
...
video.set_birdEyeViewProcessing(image_points, model_points, [-12, 4, -32.], rotation=np.array(
    [[1., 0, 0], [0, -1, 0], [0, 0, -1.]]), scale=True, framesPerSecond=30)
...
video.extractGoodFeaturesDisplacementsAccumulateAndInterpolate(
    ...
    display2='field', 
    displayColor=True,
    saveImgPath=os.path.join(CURR_PATH, 'export_tracks')
    )
...
mask = cv2.imread('1142/mask_1142.png')
A = mask > 100
video.set_stabilization(mask=A[:, :, 0], mute=False)
image_points = np.array([
    (830, 564),  # left arch
...
video.set_vlim([0, 10])
video.extractGoodFeaturesDisplacementsAccumulateAndInterpolate(
    display1='quiver', display2='field', displayColor=True)
video.set_filtersParams(maxDevInRadius=1.5, RadiusF=0.15, range_Vx=[0.01, 10])
...
parm_interp = {'kernel': 'Gaussian',
               'Radius': 5*dr,
               'Sharpness': 2.}
V1 = opyf.Interpolate.npInterpolateVTK2D(
    terrain[:, 0:2], terrain[:, 1:3], vecX, ParametreInterpolatorVTK=parm_interp)
# %
plt.close('all')
...
scale = 4
img_red = cv2.resize(image, (int(ca / scale), int(la / scale)),
...
ax.plot([OR_R[0], OR_L[0]], [OR_R[1], OR_L[1]], '-x', ms=20,
        color=[.1, 1, 0.1, 0.5], linewidth=8, markeredgewidth=4)
ax.plot([OR_R[0], OR_L[0]], [OR_R[1], OR_L[1]], '--x', ms=15,
        color=[0., 0., 0., 1], linewidth=2, markeredgewidth=1.5)
ax.text(OR_L[0]-4, OR_L[1]+0.1, 'L', color='w', fontsize=18,
        bbox=dict(facecolor='purple', zorder=3, alpha=0.6))
ax.text(OR_R[0]-4, OR_R[1]-0.3, 'R', color='w', fontsize=18,
        bbox=dict(facecolor='purple', zorder=3, alpha=0.6))
...
V = opyf.Interpolate.npInterpolateVTK2D(
    Xtot, Vtot, bathy_new[:, 0:2], ParametreInterpolatorVTK)
...
axR.plot(bathy_new[ind_im[0], 1], V[ind_im[0], 0], '--',
         linewidth=1, label='Norm Velocity ( in m/s )')

axR.plot(bathy_new[ind_im[0], 1], bathy_z[ind_im[0]],
         '-.', linewidth=1.5, label='Bathymetry ( in m )')
...
Q = -np.mean(1*(V[ind_im, 0]**2+V[ind_im, 1]**2)
             ** 0.5*(bathy_z[ind_im]))*lengthTrans
axR.legend(fontsize=9, loc=6, bbox_to_anchor=(0.1, -0.25, 0.4, 0.2))
axR.text(13, 3, 'Q = ' + format(Q, '.1f') + ' m$^3$ s$^{-1}$', fontsize=10)
axR.text(0, -3, 'L', color='w', fontsize=18,
         bbox=dict(facecolor='purple', zorder=3, alpha=0.6))
axR.text(22, -3, 'R', color='w', fontsize=18,
         bbox=dict(facecolor='purple', zorder=3, alpha=0.6))
axR.set_xlim([0, 22])
